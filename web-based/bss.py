...
class Blu:
	...
	def subscribePercent(self, node, vdevice, obj, sv):
		...
		# 	yield int(self.specialChar(self.s.recv(2048).encode('hex')[24:][:6], 1))/6400
		# 	yield round(float(struct.unpack('>l', self.specialChar(self.s.recv(2048).encode('hex')[24:], 1)[:8].decode('hex'))[0]), 2)
		# value = self.specialChar(self.s.recv(2048).encode('hex'), 1)
		# value = value[24:][:8].encode('hex')
		#value = self.specialChar(self.s.recv(2048).encode('hex')[24:], 1)[:8]
		#value = struct.unpack('>1', value)
			yield int(self.specialChar(self.s.recv(2048).encode('hex'), 1)[20:][:-4], 16)/65536

		#return int(self.specialChar(self.s.recv(2048).encode('hex'), 1)[20:][:-4], 16)/65536

		#int(self.specialChar(self.s.recv(2048).encode('hex')[24:][:6], 1))/6400
		#return round(float(struct.unpack('>l', self.specialChar(self.s.recv(2048).encode('hex')[24:], 1)[:8].decode('hex'))[0]), 2)
		# self.s.close()
	...
	def subscribeRaw(self, node, vdevice, obj, sv):
		...
		while 1:
			yield self.specialChar(self.s.recv(2048).encode('hex'), 1)[-5:][:-4]


		# 02 | 8d | 10 f4 | 03 | 00 01 00 | 01 2c | 00 64 00 00 | (chkSum)22 | (end)03
